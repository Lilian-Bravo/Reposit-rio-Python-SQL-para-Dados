...
Tabela Origem: RSTD_LMD_AR_AV_M
...
------------------------------------------------------------------- RSTD_LMD_AR_AV_M -----------------------------------------------------
SELECT count(*) FROM RSTD_LMD_AR_AV_M
WHERE CD_EVCT = 202107170000  
-- número de registros 

SELECT *FROM RSTD_LMD_AR_AV_M
WHERE CD_EVCT = 202107170000  
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT cd_evct, 
sum (VL_PAG_CCEAR_P3), sum (VL_REC_CCEAR_P3), sum(PC_PRIFC_TOT_P3)
FROM RSTD_LMD_AR_AV_M
WHERE CD_EVCT = 202107170000 and nu_parc_usin = 888069
group by cd_evct
-- and nu_parc_usin = 888069
-- Soma da quantidade de valores campo VL_PAG_CCEAR_P3:  VL_REC_CCEAR_P3:  PC_PRIFC_TOT_P3:
-- Soma da quantidade de valores para amostra campo VL_PAG_CCEAR_P3:  VL_REC_CCEAR_P3:  PC_PRIFC_TOT_P3:
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT COUNT(*) FROM FAT_S_PERF_AGCV_MLIQ_MCSD
WHERE CD_EVCT = 202107170000
-- número de registros 
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT * FROM FAT_S_PERF_AGCV_MLIQ_MCSD
WHERE CD_EVCT = 202107170000
-- Todos os registros foram carregados no campo sem valor genérico para as dimensões.
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT DISTINCT 
	...
	A.VL_REC_CCEAR_P3,
	A.PC_PRIFC_TOT_P3
FROM FAT_S_PERF_AGCV_MLIQ_MCSD  A
...
select cd_evct, 
sum (VL_PAG_CCEAR_P3),sum (VL_REC_CCEAR_P3), sum(PC_PRIFC_TOT_P3)
FROM FAT_S_PERF_AGCV_MLIQ_MCSD
WHERE CD_EVCT = 202107170000
group by cd_evct
